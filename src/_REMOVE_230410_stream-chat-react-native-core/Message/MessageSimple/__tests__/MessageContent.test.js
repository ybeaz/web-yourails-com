...
describe('MessageContent', () => {
  ...
  it('renders a MessageHeader when the prop exists', async () => {
    ...
    const ContextMessageHeader = props => (
      <SafeAreaView {...props} testID='message-header' />
    )
  ...
  it('renders a MessageFooter when the prop exists', async () => {
    ...
    const ContextMessageFooter = props => (
      <SafeAreaView {...props} testID='message-footer' />
    )
  ...
  it('renders the FileAttachment component when a file attachment exists', async () => {
    ...
    const { getByTestId, queryAllByA11yLabel, queryAllByTestId } =
    ...
    const { getByTestId } = render(
      <ChannelsStateProvider>
        <Chat client={chatClient}>
          <Channel
            channel={channel}
            MessageContent={MessageContentWithMockedMessageContentWidth}
          >
            <Message
              groupStyles={['bottom']}
              message={message}
