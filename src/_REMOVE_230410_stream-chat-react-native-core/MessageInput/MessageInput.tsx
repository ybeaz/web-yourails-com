...
>(
  ...
  useEffect(() => {
    ...
    if (fileUploads.length) {
      fileUploads.forEach((file: any) => removeFile(file.id))
    }
  ...
  useEffect(() => {
    if (imagesForInput === false && imageUploads.length) {
      imageUploads.forEach((image: any) => removeImage(image.id))
    }
  }, [imagesForInput])
  ...
  const uploadImagesHandler = () => {
    const imageToUpload = selectedImages.find((selectedImage: any) => {
    ...
    if (
      imageToUpload &&
      Number(imageToUpload.fileSize) / MEGA_BYTES_TO_BYTES >
      ...
      setSelectedImages(
        selectedImages.filter(
          (selectedImage: any) => selectedImage.uri !== imageToUpload.uri
        )
      )
    } else {
  ...
  const removeImagesHandler = () => {
    ...
    imagesToRemove.forEach((image: any) => removeImage(image.id))
  }
  ...
  useEffect(() => {
    if (selectedFilesLength > fileUploadsLength) {
      ...
      const fileToUpload = selectedFiles.find((selectedFile: any) => {
        const uploadedFile = fileUploads.find(
          (fileUpload: any) =>
            fileUpload.file.uri === selectedFile.uri ||
            fileUpload.url === selectedFile.uri
        )
        return !uploadedFile
      })
      ...
      if (
        fileToUpload &&
        Number(fileToUpload.size) / MEGA_BYTES_TO_BYTES >
          MAX_FILE_SIZE_TO_UPLOAD_IN_MB
      ) {
        Alert.alert(
        ...
        setSelectedFiles(
          selectedFiles.filter(
            (selectedFile: any) => selectedFile.uri !== fileToUpload.uri
          )
        )
      } else {
      ...
      const filesToRemove = fileUploads.filter(
        (fileUpload: any) =>
          !selectedFiles.find(
            (selectedFile: any) =>
              selectedFile.uri === fileUpload.file.uri ||
              selectedFile.uri === fileUpload.url
          )
      )
      filesToRemove.forEach((file: any) => removeFile(file.id))
    }
  }, [selectedFilesLength])
  ...
  useEffect(() => {
    if (fileUploadsLength < selectedFilesLength) {
      ...
      const updatedSelectedFiles = selectedFiles.filter((selectedFile: any) => {
        const uploadedFile = fileUploads.find(
          (fileUpload: any) =>
            fileUpload.file.uri === selectedFile.uri ||
            fileUpload.url === selectedFile.uri
       ...
       **/
      setSelectedFiles(
        fileUploads.map((fileUpload: any) => ({
