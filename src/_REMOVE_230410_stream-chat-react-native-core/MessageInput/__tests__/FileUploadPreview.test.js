...
function MockedFlatList(props) {
  const items = props.data.map((item, index) => {
    ...
    return (
      <SafeAreaView key={key}>{props.renderItem({ index, item })}</SafeAreaView>
    )
  })
  return <SafeAreaView testID={props.testID}>{items}</SafeAreaView>
}
...
describe('FileUploadPreview', () => {
  ...
  it('should render FileUploadPreview with all unsupported files', async () => {
    ...
    await waitFor(() => {
      ...
      expect(queryAllByText('File type not supported')).toHaveLength(
        fileUploads.length
      )
      expect(removeFile).toHaveBeenCalledTimes(0)
      expect(uploadFile).toHaveBeenCalledTimes(0)
  ...
  it('should render FileUploadPreview with all uploaded audios', async () => {
    ...
    const AudioAttachmentComponent = getByTestId(
      'audio-attachment-upload-preview'
    )
